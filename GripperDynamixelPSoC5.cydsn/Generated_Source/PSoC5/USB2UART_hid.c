...
volatile uint8 USB2UART_hidProtocol[USB2UART_MAX_INTERFACES_NUMBER];  /* HID device protocol status */
volatile uint8 USB2UART_hidIdleRate[USB2UART_MAX_INTERFACES_NUMBER];  /* HID device idle reload value */
volatile uint8 USB2UART_hidIdleTimer[USB2UART_MAX_INTERFACES_NUMBER]; /* HID device idle rate value */
...
/*******************************************************************************
* Function Name: USB2UART_UpdateHIDTimer
********************************************************************************
*
* Summary:
...
{
    uint8 stat USB2UART_IDLE_TIMER_INDEFINITE;

    if(USB2UART_hidIdleRate[interface] != 0u)
    {
        if(USB2UART_hidIdleTimer[interface] > 0u)
        {
            USB2UART_hidIdleTimer[interface]--;
            stat USB2UART_IDLE_TIMER_RUNNING;
        }
        else
        {
            USB2UART_hidIdleTimer[interface] USB2UART_hidIdleRate[interface];
            stat USB2UART_IDLE_TIMER_EXPIRED;
        }
    }
...
/*******************************************************************************
* Function Name: USB2UART_GetProtocol
********************************************************************************
*
* Summary:
...
{
    return(USB2UART_hidProtocol[interface]);
}


/*******************************************************************************
* Function Name: USB2UART_DispatchHIDClassRqst
********************************************************************************
*
* Summary:
    ...
    {   /* Control Read */
        switch (CY_GET_REG8(USB2UART_bRequest))
        {
            case USB2UART_GET_DESCRIPTOR:
                if (CY_GET_REG8(USB2UART_wValueHi) == USB2UART_DESCR_HID_CLASS)
                {
                    USB2UART_FindHidClassDecriptor();
                    if (USB2UART_currentTD.count != 0u)
                    {
                        requestHandled USB2UART_InitControlRead();
                    }
                }
                else if (CY_GET_REG8(USB2UART_wValueHi) == USB2UART_DESCR_HID_REPORT)
                {
                    USB2UART_FindReportDescriptor();
                    if (USB2UART_currentTD.count != 0u)
                    {
                        requestHandled USB2UART_InitControlRead();
                    }
            ...
            case USB2UART_HID_GET_IDLE:
                /* This function does not support multiple reports per interface*/
                /* Validate interfaceNumber and Report ID (should be 0) */
                if( (interfaceNumber < USB2UART_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(USB2UART_wValueLo) == 0u ) ) /* Do not support Idle per Report ID */
                {
                    USB2UART_currentTD.count 1u;
                    USB2UART_currentTD.pData &USB2UART_hidIdleRate[interfaceNumber];
                    requestHandled  USB2UART_InitControlRead();
                }
                break;
            case USB2UART_HID_GET_PROTOCOL:
                /* Validate interfaceNumber */
                if( interfaceNumber < USB2UART_MAX_INTERFACES_NUMBER)
                {
                    USB2UART_currentTD.count 1u;
                    USB2UART_currentTD.pData &USB2UART_hidProtocol[interfaceNumber];
                    requestHandled  USB2UART_InitControlRead();
                }
                break;
    ...
    {   /* Control Write */
        switch (CY_GET_REG8(USB2UART_bRequest))
        {
            case USB2UART_HID_SET_REPORT:
                ...
                break;
            case USB2UART_HID_SET_IDLE:
                /* This function does not support multiple reports per interface */
                /* Validate interfaceNumber and Report ID (should be 0) */
                if( (interfaceNumber < USB2UART_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(USB2UART_wValueLo) == 0u ) ) /* Do not support Idle per Report ID */
                {
                    USB2UART_hidIdleRate[interfaceNumber] CY_GET_REG8(USB2UART_wValueHi);
                    ...
                    */
                    if(USB2UART_hidIdleRate[interfaceNumber] <
                       USB2UART_hidIdleTimer[interfaceNumber])
                    {
                        /* Set the timer to zero and let the UpdateHIDTimer() API return IDLE_TIMER_EXPIRED status*/
                        USB2UART_hidIdleTimer[interfaceNumber] 0u;
                    }
                    ...
                    */
                    else if(USB2UART_hidIdleTimer[interfaceNumber] <= 1u)
                    {
                        /* Do nothing.
                    ...
                    {   /* Reload the timer*/
                        USB2UART_hidIdleTimer[interfaceNumber] =
                        USB2UART_hidIdleRate[interfaceNumber];
                    }
                    requestHandled USB2UART_InitNoDataControlTransfer();
                }
                break;

            case USB2UART_HID_SET_PROTOCOL:
                /* Validate interfaceNumber and protocol (must be 0 or 1) */
                if( (interfaceNumber < USB2UART_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(USB2UART_wValueLo) <= 1u) )
                {
                    USB2UART_hidProtocol[interfaceNumber] CY_GET_REG8(USB2UART_wValueLo);
                    requestHandled USB2UART_InitNoDataControlTransfer();
                }
                break;
            default:    /* requestHandled is initialized as FALSE by default */
...
void USB2UART_FindHidClassDecriptor(void) 
{
    ...
    USB2UART_currentTD.count *pDescr;
    USB2UART_currentTD.pData pDescr;
}
...
void USB2UART_FindReportDescriptor(void) 
{
    ...
    USB2UART_currentTD.count  (((uint16)pDescr[1u] << 8u) | pDescr[0u]);
    USB2UART_currentTD.pData &pDescr[2u];
}


/*******************************************************************************
* Function Name: USB2UART_FindReport
********************************************************************************
*
* Summary:
...
void USB2UART_FindReport(void) 
{
    ...
    /* `#END` */
    USB2UART_currentTD.count 0u;   /* Init not supported condition */
    pTmp USB2UART_GetConfigTablePtr(USB2UART_configuration - 1u);
    reportType CY_GET_REG8(USB2UART_wValueHi);
    interfaceN CY_GET_REG8(USB2UART_wIndexLo);
    ...
    /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_TABLE*/
    pTmp (const T_USB2UART_LUT CYCODE *) pTmp->p_list;
    if(interfaceN < USB2UART_MAX_INTERFACES_NUMBER)
    {
        /* Now use Alternate setting number */
        pTmp &pTmp[USB2UART_interfaceSetting[interfaceN]];
        /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_ALTERNATEi_HID_TABLE */
        ...
        if((reportType >= USB2UART_HID_GET_REPORT_INPUT) &&
           (reportType <= USB2UART_HID_GET_REPORT_FEATURE))
        {
            /* Get the entry proper TD (IN, OUT or Feature Report Table)*/
            pTmp &pTmp[reportType - 1u];
            reportType CY_GET_REG8(USB2UART_wValueLo);    /* Get reportID */
            /* Validate table support by the HID descriptor, compare table count with reportID */
            if(pTmp->c >= reportType)
            {
                pTD (T_USB2UART_TD *) pTmp->p_list;
                pTD &pTD[reportType];                          /* select entry depend on report ID*/
                USB2UART_currentTD.pData pTD->pData;   /* Buffer pointer */
                USB2UART_currentTD.count pTD->count;   /* Buffer Size */
                USB2UART_currentTD.pStatusBlock pTD->pStatusBlock;
            }
        }
    }
}
